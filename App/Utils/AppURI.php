...
class AppURI
{
    ...
    {
        $ss_obfs_list = Tools::getSupportParam('ss_obfs');
        if (!$is_ss) {
        ...
        if ($is_ss == 2) {
            ...
            if ($item['obfs'] == 'v2ray' || in_array($item['obfs'], $ss_obfs_list)) {
                ...
                $ssurl .= '/?plugin=' . rawurlencode($plugin) . '&group=' . Tools::base64_url_encode(Config::getInstance()->getConf('appName'));
            } else {
                $ssurl .= '/?group=' . Tools::base64_url_encode(Config::getInstance()->getConf('appName'));
            }
            $ssurl .= (Config::getInstance()->getConf('SUB.add_appName_to_ss_uri') === true
                ? '#' . rawurlencode(Config::getInstance()->getConf('appName') . ' - ' . $item['remark'])
                : '#' . rawurlencode($item['remark']));
            ...
            default:
                switch ($item['type']) {
                    case 'ss':
                        ...
                        $return = ($item['remark'] . ' = ss, ' . $item['address'] . ', ' . $item['port'] . ', encrypt-method=' . $item['method'] . ', password=' . $item['passwd'] . self::getSurgeObfs($item) . ', udp-relay=true');
                        break;
                    case 'vmess':
    ...
    public static function getQuantumultURI(array $item, bool $base64_encode = false)
    {
        ...
        switch ($item['type']) {
            case 'ss':
                ...
                $return = ($item['remark'] . ' = shadowsocks, ' . $item['address'] . ', ' . $item['port'] . ', ' . $item['method'] . ', "' . $item['passwd'] . '", upstream-proxy=false, upstream-proxy-auth=false' . self::getSurgeObfs($item) . ', group=' . Config::getInstance()->getConf('appName') . '_ss');
                break;
            case 'ssr':
                $return = ($item['remark'] . ' = shadowsocksr, ' . $item['address'] . ', ' . $item['port'] . ', ' . $item['method'] . ', "' . $item['passwd'] . '", protocol=' . $item['protocol'] . ', protocol_param=' . $item['protocol_param'] . ', obfs=' . $item['obfs'] . ', obfs_param="' . $item['obfs_param'] . '", group=' . Config::getInstance()->getConf('appName'));
                break;
            case 'vmess':
                ...
                $return = ($item['remark'] . ' = vmess, ' . $item['add'] . ', ' . $item['port'] . ', chacha20-ietf-poly1305, "' . $item['id'] . '", group=' . Config::getInstance()->getConf('appName') . '_VMess' . $tls . $obfs);
    ...
    public static function getShadowrocketURI(array $item)
    {
        ...
        switch ($item['type']) {
            case 'ss':
                if (in_array($item['obfs'], Tools::getSupportParam('ss_obfs'))) {
